        ...
        {PRODUCT_TYPES.map(t => {
          ...
          return (
            <button
              key={t.id}
              type="button"
              className={`ap-card${t.built ? '' : ' ap-card-soon'}`}
              onClick={() => pick(t)}
...
function Styles() {
  return <style>{`
    ...
    .ap-card:focus-visible { outline:none; border-color:var(--accent); box-shadow:0 0 0 3px var(--accent-light); }
    ...
    .ap-creating { font-size:12px; font-weight:700; color:var(--accent-hover); margin-top:2px; }

    .ap-err { margin-top:18px; color:var(--accent); background:var(--accent-light); border:1px solid var(--accent-mid); border-radius:var(--r-sm); padding:10px 14px; font-size:14px; }
