...
router.post('/:skillId/intent', async (req, res) => {
    try {
        ...
        const paymentIntent = await stripe.paymentIntents.create({
            amount: charge.cents,
            currency: 'usd',
            application_fee_amount: skillFeeCents(charge.cents),
            transfer_data: { destination: creator.stripe_account_id },
            automatic_payment_methods: { enabled: true }, // card + Apple/Google Pay
            metadata: { kind: 'skill', skill_id: skillId, buyer_id: buyerId, version: String(skill.version), code: charge.code || '' },
        });

        // Upsert a pending purchase (one row per buyer+skill).
        ...
        res.json({ clientSecret: paymentIntent.client_secret, paymentIntentId: paymentIntent.id, amountCents: charge.cents });
    } catch (err) {
